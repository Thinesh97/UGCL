...
public partial class Local_MRN : System.Web.UI.Page
{
        ...
        try
        {
            if (Session["UID"] != null)
            {
                ...
                if (!IsPostBack)
                {
                    ...
                    {
                        GetLocal_MRN_Details(Request.QueryString["ID"].ToString());
                        //if (rd_Status.SelectedValue == "Close")
                        //{
    ...
    protected void BindYear()
    {
        try
        {
            objLocal_MRNBL = new Local_MRNBL();
            ds = new DataSet();
            objLocal_MRNBL.load(con, Local_MRNBL.eLoadSp.SELECT_YEAR, ref ds);
    ...
    //        objLocal_MRNBL = new Local_MRNBL();
    //        ds = new DataSet();
    //        objLocal_MRNBL.load(con, Local_MRNBL.eLoadSp.SELECT_SECTOR, ref ds);
    ...
    protected void GetLocal_MRN_Details(string Local_MRN_ID)
    {
        try
        {
            objLocal_MRNBL = new Local_MRNBL();
            ds = new DataSet();
            objLocal_MRNBL.Local_MRN_ID = Convert.ToInt32(Local_MRN_ID);
            objLocal_MRNBL.load(con, Local_MRNBL.eLoadSp.SELECT_MRN_BY_ID, ref ds);
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                //ddlSector.SelectedValue = ds.Tables[0].Rows[0]["Budget_Sector_ID"].ToString();
                txt_Amt.Text = ds.Tables[0].Rows[0]["Amount"].ToString();
    ...
    protected void btnSaveBudget_Click(object sender, EventArgs e)
    {
        ...
        {
            objLocal_MRNBL = new Local_MRNBL();
            objLocal_MRNBL.Project_Code = txtProjectcode.Text;
            objLocal_MRNBL.Month_ID = Convert.ToInt32(ddlMonth.SelectedValue);
            objLocal_MRNBL.Year = Convert.ToInt32(ddlYear.SelectedValue);
            //objLocal_MRNBL.Budget_Sector_ID = Convert.ToInt32(ddlSector.SelectedValue);
            objLocal_MRNBL.Amount = Convert.ToDecimal(txt_Amt.Text);
            objLocal_MRNBL.Status = rd_Status.SelectedValue;
            if (((Button)sender).Text == "Save")
            {
                if (objLocal_MRNBL.insert(con, Local_MRNBL.eLoadSp.Insert))
                {
                    btnSaveBudget.Text = "Update";
                    ddlProject.Enabled = false;
                    ddlMonth.Enabled = false;
                    ddlYear.Enabled = false;
                    //ddlSector.Enabled = false;
                    ViewState["Local_MRN_ID"] = objLocal_MRNBL.Local_MRN_ID;
                    ScriptManager.RegisterStartupScript(this, GetType(), "CustomAlert", "Alert.render('Local MRN Added successfully');", true);
                    if (rd_Status.SelectedValue == "Close")
                    {
                        btnSaveBudget.Visible = false;
                    }
                ...
                {
                    ScriptManager.RegisterStartupScript(this, GetType(), "CustomAlert1", "Alert1.render('The Local MRN that you are trying to add already exists for the selected project.');", true);
                }
            }
            else
            {
                    if(ViewState["Local_MRN_ID"]!= null)
                    {
                        objLocal_MRNBL.Local_MRN_ID = Convert.ToInt32(ViewState["Local_MRN_ID"]);
                    }
                    else
                    {
                        objLocal_MRNBL.Local_MRN_ID = Convert.ToInt32(Request.QueryString["ID"].ToString());
                    }
                    if (objLocal_MRNBL.update(con, Local_MRNBL.eLoadSp.UPDATE))
                    {
                        ScriptManager.RegisterStartupScript(this, GetType(), "CustomAlert", "Alert.render('Local MRN has been updated');", true);
                        if (rd_Status.SelectedValue == "Close")
                        {
                            btnSaveBudget.Visible = false;
                        }
                    ...
                    {
                        ScriptManager.RegisterStartupScript(this, GetType(), "CustomAlert1", "Alert1.render('Failed to update');", true);
                    }
                }
        }
        catch (Exception ex)
