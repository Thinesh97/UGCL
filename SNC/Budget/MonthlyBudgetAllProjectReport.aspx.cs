...
using DataAccess;
    ...
    public partial class MonthlyBudgetAllProjectReport : System.Web.UI.Page
    {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                    ...
                    objBudgetBL.load(con, BudgetBL.eLoadSp.BUDGET_SEARCH_All, ref ds);
        ...
        protected void Grid_Search_RowDataBound(object sender, Obout.Grid.GridRowEventArgs e)
        {
            try
            {
                ...
                if (e.Row.RowType == Obout.Grid.GridRowType.DataRow)
                {
                    ...
                    Total_PO_Amount += decimal.Parse(e.Row.Cells[4].Text);
                }
                else if (e.Row.RowType == Obout.Grid.GridRowType.ColumnFooter)
                {
                    ...
                    e.Row.Cells[4].Text = Total_PO_Amount.ToString();
        ...
        protected void lnkbtnBudgetSector_Click(object sender, EventArgs e)
        {
            try
            {
                objBudgetBL = new BudgetBL();
                ds = new DataSet();
                objBudgetBL.Project_Code = ((LinkButton)sender).Text.ToString();
                objBudgetBL.Month = ddlProjectMonth.SelectedIndex != 0 ? Convert.ToInt16(ddlProjectMonth.SelectedValue.Trim()) : 0;
                objBudgetBL.Year = ddlProjectYear.SelectedIndex != 0 ? Convert.ToInt32(ddlProjectYear.SelectedValue.Trim()) : 0;
                objBudgetBL.load(con, BudgetBL.eLoadSp.BUDGET_SEARCH_Project_Wise, ref ds);
                if (ds.Tables[0].Rows.Count > 0)
                {
                    gridcompanywise.DataSource = ds;
                    gridcompanywise.DataBind();
                }
                else
                {
                    gridcompanywise.DataSource = null;
                    gridcompanywise.DataBind();
                }                
            }
            catch (Exception ex)
        ...
        protected void btnProjectWiseExport_Click(object sender, EventArgs e)
        {
            gridcompanywise.PageSize = -1;
            gridcompanywise.DataBind();
            ExportGridToPDF(gridcompanywise);
        }
    }
