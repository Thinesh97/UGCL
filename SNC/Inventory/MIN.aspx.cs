...
public partial class MIN : System.Web.UI.Page
{
    ...
    protected void BindContractList()
    {
        try
        {
            ...
            objSubContractorBL.load(con, SubContractorBL.eLoadSp.SELECT_CONTRACTOR_ALL, ref SubDs);
            ddlSubContractor.DataSource = SubDs;
    ...
    protected void ItemList_Grid_RowDataBound(object sender, Obout.Grid.GridRowEventArgs e)
    {
        try
        {
            if (e.Row.RowType == Obout.Grid.GridRowType.DataRow)
            {
                ...
                CheckBox chkSelect = e.Row.Cells[6].FindControl("chkSelect") as CheckBox;
                //GridDataControlFieldCell cell1 = e.Row.Cells[8] as GridDataControlFieldCell;
                TextBox txtApproveQty = (e.Row.FindControl("txtApproveQty") as TextBox);
                GridDataControlFieldCell cell2 = e.Row.Cells[8] as GridDataControlFieldCell;
                TextBox txtIssueQty = (TextBox)cell2.FindControl("txtIssueQty");
                if (e.Row.Cells[17].Text == "True")
                {

                    TextBox txtApproveQty1 = (e.Row.FindControl("txtApproveQty") as TextBox);
                    chkSelect.Enabled = true;
                    //txtApproveQty.Enabled = false;
                    txtApproveQty1.Enabled = false;
                    txtIssueQty.Enabled = true;
                    btnApprove.Visible = false;
                    btnIssueQty.Visible = true;
                }
                else
                {
                    chkSelect.Enabled = true;
                    txtIssueQty.Enabled = false;
                    txtApproveQty.Enabled = true;
                }
            }
        }
        catch (Exception ex)
    ...
    protected void GetMINLISTDetails(string MIN_No)
    {
        try
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                ddlSubContractor.SelectedValue = "0" + ds.Tables[0].Rows[0]["Subcon_ID"].ToString();
                rdRecoverable.SelectedValue = ds.Tables[0].Rows[0]["Recoverable"].ToString();
                ddlWO.SelectedItem.Text = ds.Tables[0].Rows[0]["WONo"].ToString();
                ...
                {
                    btnApprove.Visible = false;
                    btnIssueQty.Visible = false;
                    if (Session["UID"].ToString() == ds.Tables[0].Rows[0]["Approver_MIN"].ToString())
                    {
    ...
    protected void btnIssueQty_Click(object sender, EventArgs e)
    { 
        try
        {

            for (int i = 0; i < ItemList_Grid.RowsInViewState.Count; i++)
            {
                 objMINBL = new MINBL();
                 ds = new DataSet();
                GridDataControlFieldCell cell = ItemList_Grid.RowsInViewState[i].Cells[6] as GridDataControlFieldCell;
                CheckBox chkSelect = (CheckBox)cell.FindControl("chkSelect");
                HiddenField hdn_MIN_Item_ID = cell.FindControl("hdn_MIN_Item_ID") as HiddenField;
                HiddenField hdn_Item_Code = cell.FindControl("hdn_Item_Code") as HiddenField;
                GridDataControlFieldCell cell1 = ItemList_Grid.RowsInViewState[i].Cells[8] as GridDataControlFieldCell;
                TextBox txtIssueQty = (TextBox)cell1.FindControl("txtIssueQty");
                objMINBL.MIN_Item_ID =Convert.ToInt32(hdn_MIN_Item_ID.Value);
                objMINBL.ApproveQty = Convert.ToDecimal(txtIssueQty.Text.Trim());
                objMINBL.Item_Code = Convert.ToString(hdn_Item_Code.Value);
                objMINBL.Task = "IssueQty";
                if (txtMINNo.Text !="")
                {
                ...
                {
                    if (objMINBL.load(con, MINBL.eLoadSp.Update_MIN_Items_BY_ID, ref ds))
                    {
                        objMINBL = new MINBL();
                        DataSet dsStock = new DataSet();
                        objMINBL.Item_Code = Convert.ToString(hdn_Item_Code.Value);
                        objMINBL.Project_Code =Convert.ToString(ddlProjectName.SelectedValue);
                        objMINBL.Task = "Sum_Adjusted_Qty";
                        objMINBL.load(con, MINBL.eLoadSp.DEDUCT_STOCK_BASED_ON_ISSUE_QTY, ref dsStock);
                        if (true)
                        {
                            AvailiableQty = Convert.ToDecimal(dsStock.Tables[0].Rows[0]["AvailableQty"].ToString());
                        }
                        dsStock.Clear();
                        ...
                        if (Ds_Deduct_Stock.Tables[0].Rows.Count >= 0)
                        {
                            foreach (DataRow item in Ds_Deduct_Stock.Tables[0].Rows)
                            {
                                ...
                                objMINBL.Issue_Qty =Convert.ToDecimal(item["Adjust_QTY"].ToString()); 
                                objMINBL.Stock_ID = Convert.ToInt32(item["Stock_ID"]);
                                objMINBL.load(con, MINBL.eLoadSp.UPDATE_STOCK_QTY, ref ds_Update);

                            }
                            objMINBL.Issue_Qty = objMINBL.Issue_Qty + Convert.ToDecimal(txtIssueQty.Text.Trim());
                        ...
                        Response.Redirect("MINList.aspx");
                        BindItemsList();
                        ScriptManager.RegisterStartupScript(this, GetType(), "CustomAlert", "Alert.render('Item Has Been Issued');", true);
                        BindItemsList();
                    }
                    else
                    {
                        ScriptManager.RegisterStartupScript(this, GetType(), "CustomAlert1", "Alert1.render('Failed To Issued');", true);
                    }
                }
            }
        }
    ...
    { 
        try
        {
            ...
            for (int i = 0; i < ItemList_Grid.RowsInViewState.Count; i++)
            {
                ...
                CheckBox chkSelect = (CheckBox)cell.FindControl("chkSelect");
                HiddenField hdn_MIN_Item_ID = cell.FindControl("hdn_MIN_Item_ID") as HiddenField;
                GridDataControlFieldCell cell1 = ItemList_Grid.RowsInViewState[i].Cells[5] as GridDataControlFieldCell;
                TextBox txtApproveQty = (TextBox)cell1.FindControl("txtApproveQty");
                objMINBL.MIN_Item_ID =Convert.ToInt32(hdn_MIN_Item_ID.Value);
                objMINBL.ApproveQty = Convert.ToDecimal(txtApproveQty.Text.Trim());
                objMINBL.Task = "ApproveQty";
                if (txtMINNo.Text !="")
                {
                    objMINBL.MIN_No =Convert.ToString(txtMINNo.Text);
                }
                ...
                if (chkSelect != null && chkSelect.Checked)
                {
                    ...
                    {
                        Response.Redirect("MINList.aspx");
                        ScriptManager.RegisterStartupScript(this, GetType(), "CustomAlert", "Alert.render('Item Has Been Approved');", true);
                        BindItemsList();
                    }
                    else
                    {
                        ScriptManager.RegisterStartupScript(this, GetType(), "CustomAlert1", "Alert1.render('Failed To Approve');", true);
                    }
                }
            }
        }
    ...
    protected void Btn_SaveItem(object sender, EventArgs e)
    {
            try
            {
                ...
                objMINBL.Quantity = txtQty.Text != string.Empty ? Convert.ToDecimal(txtQty.Text) : Convert.ToDecimal(0);
    ...
    protected void ItemList_Grid_DeleteCommand(object sender, Obout.Grid.GridRecordEventArgs e)
    {
        ...
        catch (Exception ex)
        {
            ...
            ErrorLogger.logError(ErrorLogger.enumErrorTypes.DBDeleteError, ex, this.Page.ToString(), System.Reflection.MethodBase.GetCurrentMethod().Name, Session["UserID"] != null ? Session["UserID"].ToString() : string.Empty);
        }
    }
