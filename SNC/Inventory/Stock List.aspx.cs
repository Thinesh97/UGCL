...
public partial class Stock_List : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Session["UID"] != null)
        {
            ...
            if (!IsPostBack)
            {
                BindStock_ConsolidatedDetails();
                BindStockDetails();
                if (Accessds != null && Accessds.Tables.Count > 0 && Accessds.Tables[0].Rows.Count > 0)
                {
                    if (Accessds.Tables[0].Rows[0]["Role"].ToString() != "Application Admin" && !Convert.ToBoolean(Accessds.Tables[0].Rows[0]["Stock_View"].ToString()))
                    {
        ...
        try
        {
            ...
            if (Accessds != null && Accessds.Tables.Count > 0 && Accessds.Tables[0].Rows.Count > 0)
            {
                if (Accessds.Tables[0].Rows.Count > 0)
                {
                    ...
                    {
                        if (Convert.ToBoolean(Accessds.Tables[0].Rows[0]["Stock_Create"].ToString()))
                        {
    ...
    protected void BindStock_ConsolidatedDetails()
    {
        try
        {
            ...
            DataTable dtMRNStock = new DataTable();
            ds = new DataSet();
            objstockBL.ProjectCode = Session["Project_Code"].ToString();
            objstockBL.Task = "SelectConsolidated_Stock_List";
            objstockBL.load(con, StockBL.eLoadSp.SELECTALL_STOCK, ref ds);
            Grid_Consolidated_Stock.DataSource=ds;
            Grid_Consolidated_Stock.DataBind();
    ...
    protected void BindStockDetails()
    {
        try
        {
            ...
            ds = new DataSet();
            objstockBL.Task = "Stock_List";
            objstockBL.ProjectCode = Session["Project_Code"].ToString();
            ///objstockBL.UserID = Convert.ToInt32(Session["UID"]);
            objstockBL.load(con, StockBL.eLoadSp.SELECTALL_STOCK, ref ds);

            DatafilterDt = ds.Tables[0];
            dtStock = DatafilterDt.Clone();
            dtMRNStock = DatafilterDt.Clone();

            foreach (DataRow row in DatafilterDt.Rows)
            {
                object value = row["MRN_NO"];
                if (value == DBNull.Value)
                {
                    dtStock.Rows.Add(row.ItemArray);
                }
                else
                {
                    dtMRNStock.Rows.Add(row.ItemArray);
                }
            }
            if (dtStock.Rows.Count > 0)
            {
                Grid_StockDetails.DataSource = dtStock;
            ...
            if (dtMRNStock.Rows.Count > 0)
            {
                Grid_MRNStockList.DataSource = dtMRNStock;
                Grid_MRNStockList.DataBind();
            }
            else
            {
                Grid_MRNStockList.DataSource = dtMRNStock;
    ...
    protected void btnExportToPDF_Click(object sender, EventArgs e)
    {
        try
        {
            Grid_StockDetails.PageSize = -1;
            Grid_StockDetails.DataBind();
            ExportGridToPDF(Grid_StockDetails);
        }
        catch (Exception ex)
        {
    ...
    private void ExportGridToPDF(Obout.Grid.Grid GirdData)
    {
        ...
        Response.Clear();
        Response.AddHeader("content-disposition", "attachment;filename=StocktLists.pdf");
    ...
    protected void btnMRNStockListExportToPDF_Click(object sender, EventArgs e)
    {
        try
        {
            Grid_MRNStockList.PageSize = -1;
            Grid_MRNStockList.DataBind();
            ExportGridToPDF(Grid_MRNStockList);
        }
        catch (Exception ex)
        {
            ErrorLogger.logError(ErrorLogger.enumErrorTypes.FileDownloadError, ex, this.Page.ToString(), System.Reflection.MethodBase.GetCurrentMethod().Name, Session["UID"] != null ? Session["UID"].ToString() : string.Empty);
    ...
    protected void Grid_StockDetails_RowDataBound(object sender, Obout.Grid.GridRowEventArgs e)
    {
        try
        {
            if (e.Row.RowType == Obout.Grid.GridRowType.DataRow)
            {
                HyperLink Editlink = e.Row.Cells[0].FindControl("lnkStockID") as HyperLink;

                Editlink.NavigateUrl = "~/Inventory/Stock.aspx?ID=" + Editlink.Text;

                if (Accessds != null && Accessds.Tables.Count > 0 && Accessds.Tables[0].Rows.Count > 0)
                {
                    if (Accessds.Tables[0].Rows[0]["Role"].ToString() != "Application Admin")
                    {
                        if (!Convert.ToBoolean(Accessds.Tables[0].Rows[0]["Stock_Update"].ToString()))
                        {
