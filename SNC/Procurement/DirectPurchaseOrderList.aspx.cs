...
using System.Web.UI.WebControls;
    ...
    public partial class DirectPurchaseOrderList : System.Web.UI.Page
    {
    ...
    protected void BindPurchaseOrderPendingList()
    {
        try
        {
            if (Session["Role"].ToString() == "Application Admin")
            {
                ...
                objViewPOBL = new PurchaseOrderBL();
                objViewPOBL.ProjectCode = Session["Project_Code"].ToString();
                objViewPOBL.Task = "DirectPO_Pending";
                objViewPOBL.load(con, PurchaseOrderBL.eLoadSp.PRO_PURCHASE_ORDER_SELECT_ALL, ref ds);
                Grid_PO_Pending.DataSource = ds;
                Grid_PO_Pending.DataBind();
            }
            else if (Session["Role"].ToString() == "Other")
            {
                ds = new DataSet();
                objViewPOBL = new PurchaseOrderBL();
                objViewPOBL.ProjectCode = Session["Project_Code"].ToString();
                objViewPOBL.Task = "DirectPO_Pending_Other";
                objViewPOBL.load(con, PurchaseOrderBL.eLoadSp.PRO_PURCHASE_ORDER_SELECT_ALL, ref ds);
                Grid_PO_Pending.DataSource = ds;
                Grid_PO_Pending.DataBind();
            }
    ...
    protected void btnExportToPDF_Click(object sender, EventArgs e)
        ...
        protected void Grid_PO_RowDataBound(object sender, Obout.Grid.GridRowEventArgs e)
        {
            try
            {
                ...
                {
                    HyperLink Editlik = e.Row.Cells[0].FindControl("lnkPONo") as HyperLink;
                    Editlik.NavigateUrl = "~/Procurement/DirectPurchaseOrder.aspx?PONo=" + Editlik.Text;
                    if (Accessds != null && Accessds.Tables.Count > 0 && Accessds.Tables[0].Rows.Count > 0)
                    {
                        if (Accessds.Tables[0].Rows[0]["Role"].ToString() != "Application Admin")
                        {
                    ...
                    LinkButton lnkPO_Delete = e.Row.Cells[14].FindControl("lnkPO_Delete") as LinkButton;
    ...
    protected void Grid_PO_RowDataBound_Pending(object sender, Obout.Grid.GridRowEventArgs e)
    {
        try
        {
            if (e.Row.RowType == Obout.Grid.GridRowType.DataRow)
    ...
    protected void Grid_PO_DeleteCommand(object sender, Obout.Grid.GridRecordEventArgs e)
        {
            try
            {
                objViewPOBL = new PurchaseOrderBL();
                objViewPOBL.PONo = e.Record["PONo"].ToString();
