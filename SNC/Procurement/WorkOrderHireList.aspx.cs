...
public partial class WorkOrderHireList : System.Web.UI.Page
{
    ...
    protected void BindWorkOrderList()
    {
        try
        {
            if (Session["Role"].ToString() == "Application Admin")
            {
                ...
                objViewWOBL.Task = "HireWorkOrderList";
            ...
            else if (Session["Role"].ToString() == "Other")
            {
                ...
                objViewWOBL = new WorkOrderBL();
                objViewWOBL.ProjectCode = Session["Project_Code"].ToString();
                objViewWOBL.Task = "HireWorkOrderList_Other";
                objViewWOBL.load(con, WorkOrderBL.eLoadSp.PRO_WORK_ORDER_SELECT_ALL, ref ds);
                Grid_WO.DataSource = ds;
                Grid_WO.DataBind();
            }
    ...
    protected void BindWorkOrderListPending()
    {
        try
        {
            if (Session["Role"].ToString() == "Application Admin")
            {
                ...
                objViewWOBL = new WorkOrderBL();
                objViewWOBL.ProjectCode = Session["Project_Code"].ToString();
                objViewWOBL.Task = "HireWorkOrderList_Pending";
                objViewWOBL.load(con, WorkOrderBL.eLoadSp.PRO_WORK_ORDER_SELECT_ALL, ref ds);
                Grid_WO_Pending.DataSource = ds;
                Grid_WO_Pending.DataBind();
            }
            else if (Session["Role"].ToString() == "Other")
            {
                ds = new DataSet();
                objViewWOBL = new WorkOrderBL();
                objViewWOBL.ProjectCode = Session["Project_Code"].ToString();
                objViewWOBL.Task = "HireWorkOrderList_Pending_Other";
                objViewWOBL.load(con, WorkOrderBL.eLoadSp.PRO_WORK_ORDER_SELECT_ALL, ref ds);
                Grid_WO_Pending.DataSource = ds;
                Grid_WO_Pending.DataBind();
            }
    ...
    protected void Grid_WO_RowDataBound(object sender, Obout.Grid.GridRowEventArgs e)
    {
        try
        {
            if (e.Row.RowType == Obout.Grid.GridRowType.DataRow)
            {
                ...
                Editlik.NavigateUrl = "~/Procurement/WorkOrderHire.aspx?WONo=" + Editlik.Text;
                if (Accessds != null && Accessds.Tables.Count > 0 && Accessds.Tables[0].Rows.Count > 0)
                {
                    if (Accessds.Tables[0].Rows[0]["Role"].ToString() != "Application Admin")
                    {
    ...
    protected void Grid_WO_DeleteCommand(object sender, Obout.Grid.GridRecordEventArgs e)
    {
        try
        {
            ...
            {
                BindWorkOrderList();
                ScriptManager.RegisterStartupScript(this, GetType(), "CustomAlert", "Alert.render('Hire Order has been deleted successfully');", true);
            }
            else
            {
                ScriptManager.RegisterStartupScript(this, GetType(), "CustomAlert1", "Alert1.render('Hire Order is refered other process!');", true);
            }
        }
        catch (Exception ex)
        {
    ...
    protected void lnkWO_Delete_Click(object sender, CommandEventArgs e)
    {
        try
        {
            if (HF_Confirm.Value != "false")
            {
                ...
                objViewWOBL.Task = "DeleteWorkOrderHire";
                if (objViewWOBL.delete(con, WorkOrderBL.eLoadSp.WO_Delete))
                {
                    BindWorkOrderList();
                    ScriptManager.RegisterStartupScript(this, GetType(), "CustomAlert", "Alert.render('Hired Order has been deleted successfully');", true);
                }
                else
                {
                    ScriptManager.RegisterStartupScript(this, GetType(), "CustomAlert1", "Alert1.render('Hired Order is refered other process!');", true);
                }
            }
        }
        catch (Exception ex)
