...
{
    public partial class RateCradList : System.Web.UI.Page
    {
        SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["SNC_DB_Connection"].ConnectionString);
        WorkOrderBL objViewWOBL = null;
        SubContractorBillBL objSCBillBL = null;
        DataSet ds = null;
        ...
        protected void BindRCList()
        {
            try
            {
                ...
                objSCBillBL = new SubContractorBillBL();
                objSCBillBL.Project_Code = Session["Project_Code"].ToString();
                objSCBillBL.Task = "Select_RateCard_List";
                objSCBillBL.load(con, SubContractorBillBL.eLoadSp.SUB_CONTRACTOR_BILL_OPERATIONS, ref ds);
                Grid_RC_List.DataSource = ds;
                Grid_RC_List.DataBind();
            }
            catch (Exception ex)
            {
                SNC.ErrorLogger.ErrorLogger.logError(SNC.ErrorLogger.ErrorLogger.enumErrorTypes.AppLogicError, ex, this.Page.ToString(), System.Reflection.MethodBase.GetCurrentMethod().Name, Session["UID"] != null ? Session["UID"].ToString() : string.Empty);
        ...
        protected void btnExWOrtToPDF_Click(object sender, EventArgs e)
        {
            try
            {
                Grid_RC_List.PageSize = -1;
                Grid_RC_List.DataBind();
                ExWOrtGridToPDF(Grid_RC_List);
            }
            catch (Exception ex)
            {
                SNC.ErrorLogger.ErrorLogger.logError(SNC.ErrorLogger.ErrorLogger.enumErrorTypes.AppLogicError, ex, this.Page.ToString(), System.Reflection.MethodBase.GetCurrentMethod().Name, Session["UID"] != null ? Session["UID"].ToString() : string.Empty);
        ...
        protected void Grid_RC_RowDataBound(object sender, Obout.Grid.GridRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == Obout.Grid.GridRowType.DataRow)
                {
                    HyperLink Editlik = e.Row.Cells[0].FindControl("lnkDPR_No") as HyperLink;
                    Editlik.NavigateUrl = "~/SubContractorBills/RateCard.aspx?RC_No=" + Editlik.Text;
                    if (Accessds != null && Accessds.Tables.Count > 0 && Accessds.Tables[0].Rows.Count > 0)
                    {
                        if (Accessds.Tables[0].Rows[0]["Role"].ToString() != "Application Admin")
                        {
                    ...
                    //{
                    LinkButton lnkWO_Delete = e.Row.Cells[6].FindControl("lnkDPR_Delete") as LinkButton;
